...
import { LinksContext } from "./LinksList";

interface Tag {
  name: string;
}

export function LinksFilterMenu() {
  const tags = trpc.useQuery(["tags.allTags"]);
  const { data, isLoading, isError } = tags;
  const { tag, setTag } = React.useContext(LinksContext);

  if (isLoading || isError) return null;

  const allowedTags = ["Website", "Reading", "Portfolio"];

  const filtered = data?.allTags.filter(
    (t: any) => allowedTags.indexOf(t.name) >= 0
  );
